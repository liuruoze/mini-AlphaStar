...
# The china API_BASE_URL is 'https://www.battlenet.com.cn/',
# and china TOKEN_URL = 'https://www.battlenet.com.cn/oauth/token'.
# But these two URLs don't provide SC2 replays (maybe Blizzard doesn't store SC2 replays in the China server).
# So for Chinese users, directly use your user client in battlenet.com.cn,
# get the Client ID and Client Secret (how to get it please see README.MA where 
# have a guide for it), pass into the API and get the replays from us.api.blizzard.com.
...
def download(key, secret, version, replays_dir, download_dir, extract=False,
    ...
    if mpyq is not None and filter_version != 'keep':
        ...
        def found_str(): return ', '.join('{}: {}'.format(v, c)
